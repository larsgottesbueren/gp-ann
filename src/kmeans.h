...
int Top1Neighbor(const PointSet& P, float* Q) {
	int best = -1;
...
void NearestCenters(const PointSet& P, const PointSet& centroids, std::vector<int>& closest_center) {
	parlay::parallel_for(0, P.n, [&](size_t i) {
		closest_center[i] = Top1Neighbor(centroids, P.GetPoint(i));
	});
}

void AggregateClusters(const PointSet& P, PointSet& centroids, std::vector<int>& closest_center) {
...
void KMeans(const PointSet& P, PointSet& centroids, size_t d, size_t n) {
	centroids.coordinates.assign(centroids.coordinates.size(), 0.f);
