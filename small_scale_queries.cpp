...
int main(int argc, const char* argv[]) {
    ...
    time = timer.Stop();
    std::cout << "HSNW routing took " << time << " seconds. That's " << 1000.0 * time / queries.n << "ms per query, or " << queries.n / time << " QPS"
              << std::endl;
    ...
    for (const auto& [desc, routing_time, probes] : probes_v) {
        ...
        for (int num_probes = 1; num_probes <= num_shards; ++num_probes) {
            ...
            std::cout << "router = " << desc << " query = IVF "
                      << "nprobes = " << num_probes << " recall = " << recall << " time = " << time << " avg latency = " << 1000.0 * latency << " ms"
                      << std::endl;
            out << part_method << "," << desc << ","
                << "BruteForce"
                << "," << num_probes << "," << latency << "," << routing_time / queries.n << "," << time / queries.n << "," << recall << std::endl;
        }
